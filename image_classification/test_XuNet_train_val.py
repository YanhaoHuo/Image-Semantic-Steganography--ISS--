...
from Models.Criterion import criterion3
from Models.RED_CNN_add import sc_encoder, sc_decoder, Quantization
from Models.SD_model_add import DenseEncoder, DenseDecoder, XuNet, ReDecoder, accuracy
...
def _transmit(im, payload, is_train=False, shuffle=False):
    imc = SC_encoder(im)  # 60
    ims = Ser_net(imc.detach(), payload.detach())  # 隐写
    if shuffle:
...
def _fit_critic1(train_data, metrics, l, device):
    gc.collect()
    critic1.train()
    SC_encoder.eval()
    SC_decoder.eval()
    ...
    for im, _ in pbar:
        ...
        imc, ims = _transmit(im, payload, False, True)
        # score
        imc_max = torch.round(torch.max(torch.abs(imc)))
        if i < num:
            im_train, label_train = sr_Net_data((imc/imc_max).float()[0:l+1], (ims/imc_max).float()[0:l+1])
            i += 1
        else:
            im_train, label_train = sr_Net_data((imc/imc_max).float()[0:l], (ims/imc_max).float()[0:l]) # normalization
        im_train = im_train.to(device)
        label_train = label_train.to(device)
        label_out = critic1(im_train)
        loss1 = criterion3(label_out, label_train)
        acc1 = accuracy(label_out, label_train)
        ...
        metrics['t.sr_loss'].append(loss1.item())
        metrics['t.acc1'].append(acc1.item())


def _fit_critic2(train_data, metrics, l, device):
    gc.collect()
    critic1.train()
    critic2.train()
    SC_encoder.eval()
    ...
    for im, _ in pbar:
        ...
        imc, ims = _transmit(im, payload, False,True)
        ##### channel #######
        im_c, im_s, _, _ = _receiver(imc, ims, False)
        # score
        if i < num:
            im_train, label_train = sr_Net_data(im_c.float()[0:l+1], im_s.float()[0:l+1])
            i += 1
        else:
            im_train, label_train = sr_Net_data(im_c.float()[0:l], im_s.float()[0:l])
        im_train = im_train.to(device)
        label_train = label_train.to(device)
        label_out = critic2(im_train)
        loss2 = criterion3(label_out, label_train)
        acc2 = accuracy(label_out, label_train)
        ...
        metrics['t.sr_loss2'].append(loss2.item())
        metrics['t.acc2'].append(acc2.item())
...
def _val(validate, metrics, device):
    ...
    for im, _ in pbar:
        with torch.no_grad():
            ...
            # score
            imc_max = torch.round(torch.max(torch.abs(imc)))
            im_train, label_train = sr_Net_data((imc/imc_max).float(), (ims/imc_max).float())
            im_train = im_train.to(device)
            label_train = label_train.to(device)
            label_out = critic1(im_train)
        ...
        pbar.set_description(
            f"acc1: {acc1.item():.5f}; acc2: {acc2.item():.5f} "
        )
        metrics['v.sr_loss'].append(sr_loss.item())
        metrics['v.acc1'].append(acc1.item())
        metrics['v.sr_loss2'].append(sr_loss2.item())
        metrics['v.acc2'].append(acc2.item())


METRIC_FIELDS = [
    't.sr_loss', 'v.sr_loss',
    't.sr_loss2', 'v.sr_loss2',
    't.acc1', 'v.acc1',
    't.acc2', 'v.acc2',
]
...
def train(train_data, valid_data):
    ...
    with open(f'./results/Sema_add2_XuNet{bpp}.csv', 'a', newline='', encoding='utf-8') as f:
        f.truncate(0)
    print(l * 100+num)
    for epoch in range(num_epochs):
        print('Epoch {}/{}'.format(epoch, num_epochs))
        # train
        _fit_critic1(train_data, metrics, l, device)
        _fit_critic2(train_data, metrics, l, device)
        if (epoch + 1) % 10 == 0:
            # val
            ...
            with open(f'./results/Sema_add2_XuNet{bpp}.csv', 'a', newline='', encoding='utf-8') as f:
                ...
                #fieldnames = [f'{field}_mean' for field in METRIC_FIELDS]
            ...
            torch.save(checkpoint, f'./model/XuNetA{bpp}{k1}{k2}_%s.pth' % (str(epoch)))
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load(
        f'./model/SD_add2{bpp}{k1}{k2}_%s.pth' % (str(best_epoch)))  # Load
    ...
    #     f'./model/XuNetA{bpp}{k1}{k2}_%s.pth' % (str(149)))  # Load breakpoint
